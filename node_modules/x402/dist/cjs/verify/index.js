...
var VerifyError = class extends Error {
  ...
  constructor(statusCode, response) {
    super(
      response.invalidReason ? `Failed to verify payment: ${response.invalidReason}` : `Failed to verify payment: ${statusCode}`
    );
...
var SettleError = class extends Error {
  ...
  constructor(statusCode, response) {
    super(
      response.errorReason ? `Failed to settle payment: ${response.errorReason}` : `Failed to settle payment: ${statusCode}`
    );
...
function useFacilitator(facilitator) {
  ...
  async function list2(config2 = {}) {
    ...
    const urlParams = new URLSearchParams(
      Object.entries(config2).filter(([_, value]) => value !== void 0).map(([key, value]) => [key, String(value)])
    );
