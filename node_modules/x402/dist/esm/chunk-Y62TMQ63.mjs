...
async function verify(client, payload, paymentRequirements) {
  ...
  if (isSmartWallet) {
    ...
    if (!bytecode || bytecode === "0x") {
      const erc6492Data = parseErc6492Signature(exactEvmPayload.signature);
...
async function settle(wallet, paymentPayload, paymentRequirements) {
  ...
  let tx;
  if (isSmartWallet) {
    const { signature: unwrappedSignature } = parseErc6492Signature(payload.signature);
...
//# sourceMappingURL=chunk-Y62TMQ63.mjs.map